...
class BoxShadowBenchmark {
  ...
  fun homeFirstRender() {
    ...
    report("Home", boxes)
    assertEquals(27, BoxShadowDiagnostics.snapshot().liveCachedBitmapCount)
  }
  ...
  fun gradientBuilderFirstRender() {
    ...
    assertEquals(14, BoxShadowDiagnostics.snapshot().liveCachedBitmapCount)
  }

  @Test
  fun layoutStressFirstRender() {
    report("Layout Stress", List(108) { Box(180, 80, 0, null) })
    assertEquals(0, BoxShadowDiagnostics.snapshot().rasterizations)
  }

  @Test
  fun repeatedDrawAndReplacementAccounting() {
    val view = shadowView(Box(340, 220, 20, "0px 8px 24px rgba(15, 23, 42, 0.18)"))
    val target = Bitmap.createBitmap(1200, 600, Bitmap.Config.ARGB_8888)
    val canvas = Canvas(target)
    draw(view, canvas, 340, 220)
    draw(view, canvas, 340, 220)
    var snapshot = BoxShadowDiagnostics.snapshot()
    ...
    snapshot = BoxShadowDiagnostics.snapshot()
    assertEquals(1, snapshot.cacheReplacements)
    assertEquals(1, snapshot.abandonedBitmapCount)
    assertTrue(snapshot.abandonedBitmapBytes > 0)
    target.recycle()
  }
  ...
  private fun report(name: String, boxes: List<Box>) {
    ...
    Log.i(
      ...
      "$name firstFrameMs=${elapsed / 1_000_000.0} " +
        "bitmapCreations=${stats.bitmapCreations} createdBytes=${stats.bitmapBytesCreated} " +
        "liveBitmaps=${stats.liveCachedBitmapCount} liveBytes=${stats.liveCachedBitmapBytes} " +
